...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <a
        href="/map"
        class="inline-flex items-center gap-1 text-sm text-muted hover:text-text transition-colors mb-6"
    >
        &larr; Back to map
    </a>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_24rem]">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-xl border border-border bg-surface p-6">
                <h1 class="text-2xl font-bold italic text-text mb-1">
            ...
            {{-- Location card --}}
            <div class="rounded-xl border border-border bg-surface p-6">
                <h2 class="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Location</h2>

                <dl class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    @if ($occurrence->country)
                        <div>
                            <dt class="text-muted text-xs mb-0.5">Country</dt>
                    ...
                    @if ($occurrence->state)
                        <div>
                            <dt class="text-muted text-xs mb-0.5">State / Province</dt>
                    ...
                    @if ($occurrence->formation)
                        <div>
                            <dt class="text-muted text-xs mb-0.5">Formation</dt>
                    ...
                    @if ($occurrence->environment)
                        <div>
                            <dt class="text-muted text-xs mb-0.5">Environment</dt>
                    ...
                    @if ($occurrence->lat !== null && $occurrence->lng !== null)
                        <div>
                            <dt class="text-muted text-xs mb-0.5">Coordinates</dt>
            ...
            {{-- Age card --}}
            <div class="rounded-xl border border-border bg-surface p-6">
                <h2 class="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Age</h2>
                ...
                @if ($occurrence->maxMa !== null && $occurrence->minMa !== null)
                    ...
                    @endphp
                    <div class="relative h-3 rounded-full bg-surface-sunken overflow-hidden">
                        <div
                            class="absolute top-0 h-full rounded-full bg-accent"
            ...
            {{-- Collection card --}}
            <div class="rounded-xl border border-border bg-surface p-6">
                <h2 class="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Collection</h2>
            ...
            @else
                <div class="h-96 w-full rounded-xl border border-border bg-surface-sunken flex items-center justify-center">
                    <div class="text-center">
                        <p class="text-sm font-medium text-text">Location not available</p>
                        <p class="text-xs text-muted mt-1">No coordinates recorded for this occurrence.</p>
                    </div>
                </div>
            @endif
        </div>
