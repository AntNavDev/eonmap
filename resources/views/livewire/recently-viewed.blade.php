...
<x-ui.dropdown>
    <x-slot:trigger>
        <button
            class="flex items-center gap-1 text-sm font-medium text-muted hover:text-text transition-colors"
            aria-haspopup="true"
            :aria-expanded="open"
        >
            Recently Viewed
    ...
    @if (count($items) === 0)
        <p class="px-4 py-3 text-sm text-muted">Nothing viewed yet.</p>
    @else
        @foreach ($items as $item)
            <x-ui.dropdown-link href="/occurrences/{{ $item['occurrence_no'] }}">
                {{ $item['name'] }}
                <span class="text-xs text-muted">(#{{ $item['occurrence_no'] }})</span>
            </x-ui.dropdown-link>
        @endforeach
    @endif
</x-ui.dropdown>
